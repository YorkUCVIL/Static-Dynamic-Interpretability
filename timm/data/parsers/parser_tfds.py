""" Dataset parser interface that wraps TFDS dataset_factory

Wraps many (most?) TFDS image-classification dataset_factory
...
try:
    ...
    print(e)
    print("Please install tensorflow_datasets package `pip install tensorflow-dataset_factory`.")
    exit(1)
...
class ParserTfds(Parser):
    ...
    There several things to be aware of:
      ...
      * The sharding (splitting of dataset into TFRecord) files imposes limitations on the number of
        replicas and dataloader workers you can use. For really small dataset_factory that only contain a few shards
        you may have to train non-distributed w/ 1-2 dataloader workers. This is likely not a huge concern as the
        benefit of distributed training or fast dataloading should be much less for small dataset_factory.
      * This wrapper is currently configured to return individual, decompressed image samples from the TFDS
        dataset. The augmentation (transforms_old) and batching is still done in PyTorch. It would be possible
    ...
    def __init__(self, root, name, split='train', shuffle=False, is_training=False, batch_size=None):
        ...
        # NOTE: please use tfds command line app to download & prepare dataset_factory, I don't want to call
