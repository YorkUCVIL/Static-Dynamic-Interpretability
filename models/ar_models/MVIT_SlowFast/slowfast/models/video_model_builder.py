...
import models.ar_models.MVIT_SlowFast.slowfast.utils.weight_init_helper as init_helper
from models.ar_models.MVIT_SlowFast.slowfast.models.attention import MultiScaleBlock
from models.ar_models.MVIT_SlowFast.slowfast.models.batchnorm_helper import get_norm
from models.ar_models.MVIT_SlowFast.slowfast.models.stem_helper import PatchEmbed
from models.ar_models.MVIT_SlowFast.slowfast.models.utils import round_width
...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # MY CODE
        num_patches = self.patch_dims[0]
        for i in range(len(self.patch_dims)):
            try:
                num_patches = num_patches * self.patch_dims[i+1]
            except:
                continue
    ...
    def forward(self, x, pool=True, cls=False, stage=None):
        x = x[0]
        ...
        if self.sep_pos_embed:
            ...
            x = x + pos_embed_cls # RuntimeError: The size of tensor a (32769) must match the size of tensor b (25089) at non-singleton dimension 1
        else:
            x = x + self.pos_embed
        ...
        # BREAKS IF SHAPE NOT 768
        # x = self.norm(x)
        if self.cls_embed_on and not pool:
            x = x[:, 0]
        else:
            x = x.mean(1)
