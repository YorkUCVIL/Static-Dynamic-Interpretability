...
import models.ar_models.SlowFast.slowfast.utils.weight_init_helper as init_helper
...
class SlowFast(nn.Module):
    ...
    def forward(self, x, bboxes=None, cls=False, path='cat', stage=None, fuse=False):
        # slowfast
        x = self.s1(x)
        if stage == 1:
            if fuse:
...
class ResNet(nn.Module):
    ...
    def forward(self, x, bboxes=None, cls=False, stage=5):
        # shape = b*c*t*h*w
        # c3d, i3d, i3d_nln, slow_only
        x = self.s1(x)
        if stage == 1:
            x = nn.AvgPool3d((x[0].shape[2], x[0].shape[3], x[0].shape[4]))(x[0])
            x = x.view(x.size(0), -1)
