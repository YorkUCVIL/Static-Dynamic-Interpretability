...
import models.ar_models.pytorchvideo.pytorchvideo.transforms.functional
import torch
...
class ApplyTransformToKey:
    ...
    Example:
        >>>   transforms_old.ApplyTransformToKey(
        >>>       key='video',
...
class UniformTemporalSubsample(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``pytorchvideo.transforms_old.functional.uniform_temporal_subsample``.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return models.ar_models.pytorchvideo.pytorchvideo.transforms.functional.uniform_temporal_subsample(
            x, self._num_samples
        )
...
class ShortSideScale(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``pytorchvideo.transforms_old.functional.short_side_scale``.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): video tensor with shape (C, T, H, W).
        """
        return models.ar_models.pytorchvideo.pytorchvideo.transforms.functional.short_side_scale(x, self._size)


class RandomShortSideScale(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``pytorchvideo.transforms_old.functional.short_side_scale``. The size
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return models.ar_models.pytorchvideo.pytorchvideo.transforms.functional.short_side_scale(x, size)


class UniformCropVideo(torch.nn.Module):
    """
    ``nn.Module`` wrapper for ``pytorchvideo.transforms_old.functional.uniform_crop``.
    """
    ...
    def __call__(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Args:
            x (Dict[str, torch.Tensor]): video clip dict.
        """
        x[self._video_key] = models.ar_models.pytorchvideo.pytorchvideo.transforms.functional.uniform_crop(
