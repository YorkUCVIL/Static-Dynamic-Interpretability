...
class Interactive(nn.Module):
    ...
    def spatial_decoder(self, x, spatial_f, temporal_f, stages=None, interm_out={}):
        requested_stages = []
        requested_streams = []
        for stage in stages:
        ...
        interm_out.update(
            set_stage_stream("layer4", requested_stages, requested_streams, None, None, feature4)
        )

        B, C, H, W = spatial_f[2].size()
        fused3 = self.Transformerstafm3(self.spatial_net.bridge3(spatial_f[2]), self.temporal_net.bridge3(temporal_f[2]))
        interm_out.update(
            set_stage_stream("layer3", requested_stages, requested_streams, None, None, fused3)
        )
        feature3 = self.spatial_net.decoder3(
            torch.cat((F.interpolate(feature4, (H, W), mode='bilinear', align_corners=True), fused3), 1))
        ...
        fused2 = self.Transformerstafm2(self.spatial_net.bridge2(spatial_f[1]), self.temporal_net.bridge2(temporal_f[1]))
        interm_out.update(
            set_stage_stream("layer2", requested_stages, requested_streams, None, None, fused2)
        )

        feature2 = self.spatial_net.decoder2(
            torch.cat((F.interpolate(feature3, (H, W), mode='bilinear', align_corners=True),fused2), 1))
...
#                                              self.temporal_net.bridge1(temporal_f[0]))), 1))
        fused1 = self.Transformerstafm1(self.spatial_net.bridge1(spatial_f[0]), self.temporal_net.bridge1(temporal_f[0]))
        interm_out.update(
            set_stage_stream("layer1", requested_stages, requested_streams, None, None, fused1)
        )

        feature1 = self.spatial_net.decoder1(
            torch.cat((F.interpolate(feature2, (H, W), mode='bilinear', align_corners=True), fused1), 1))
        ...
        return [out1, out2, out3, out4], interm_out
    ...
    def encoder(self, x, flow, stages=None):
        interm_out = {}
        ...
        spatial_c1 = self.spatial_net.inconv[0](x.view(B*Seq, C, H, W))
        temporal_c1= self.temporal_net.inconv[0](flow.view(B*Seq, C*2, H, W))

        _, C, H, W = spatial0.size()
        interm_out.update(
            set_stage_stream("conv1", requested_stages, requested_streams, spatial_c1, temporal_c1, None)
        )
        ...
        interm_out.update(
            set_stage_stream("layer1", requested_stages, requested_streams, h1_spatial, h1_temporal, None)
        )
        ...
        interm_out.update(
            set_stage_stream("layer2", requested_stages, requested_streams, h2_spatial, h2_temporal, None)
        )
        ...
        interm_out.update(
            set_stage_stream("layer3", requested_stages, requested_streams, h3_spatial, h3_temporal, None)
        )
        ...
        interm_out.update(
            set_stage_stream("layer4", requested_stages, requested_streams, h4_spatial, h4_temporal, None)
        )
    ...
    def forward(self, img, stages=None):
        flow = img['Flow']
        img = img['Image']

        spatial, spatial_feature, temporal, temporal_feature, interm_out = self.encoder(img, flow, stages=stages)
        return self.spatial_decoder(spatial, spatial_feature, temporal_feature, stages=stages, interm_out=interm_out)
                #, self.temporal_decoder(temporal, temporal_feature)
